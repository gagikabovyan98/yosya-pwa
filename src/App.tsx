...
export default function App() {
  ...
  // ✅ FIX iOS PWA/Safari: убираем "черную полосу" снизу.
  // Вместо 100vh/100dvh используем реальную высоту visualViewport и пишем в CSS var --app-height.
  useEffect(() => {
    const setAppHeight = () => {
      const vv = window.visualViewport;
      const realH = Math.round(vv?.height ?? window.innerHeight);
      document.documentElement.style.setProperty("--app-height", `${realH}px`);
    };
  ...
  // ✅ Твоя логика интро: открыл app -> интро, ушел в фон -> вернулся -> интро
  useEffect(() => {
    const onVisibility = () => {
      if (document.visibilityState === "hidden") return;
      setShowIntro(true);
  ...
  // ✅ Свайп на интро: блокируем "скролл страницы" только пока интро открыто,
  // чтобы жест шел в IntroScreen, а не уезжал экран Safari.
  useEffect(() => {
    if (!showIntro) return;

    const prevent = (e: TouchEvent) => {
      if (e.touches && e.touches.length > 1) return; // pinch не трогаем
      e.preventDefault();
    };

    // блокируем только touchmove; passive:false обязательно
    document.addEventListener("touchmove", prevent, { passive: false });
