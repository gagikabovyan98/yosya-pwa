 ...
 *
 * IMPORTANT: Do NOT recreate/revoke URL on every refresh based on Blob identity.
 * IndexedDB often returns a NEW Blob instance each read, even if bytes are same.
 * That causes iOS to show blue "?" placeholders when URLs are revoked mid-render.
 *
 * Strategy:
 * - create URL only once per id
 * - revoke only when item disappears (deleted) or on unmount
 *
 * If later you add real "image edit/replace" for SAME id, then:
 * - either create a new id for edited photo, OR
 * - store a version/updatedAt field and recreate URL only when version changes.
 */
function usePhotoUrls(items: PhotoRecord[]) {
  ...
  useEffect(() => {
    ...
    // add urls for new ids (do NOT recreate for existing ids)
    for (const it of items) {
      if (!map.has(it.id)) {
        map.set(it.id, URL.createObjectURL(it.blob));
      }
    }
  ...
  return (id: string) => mapRef.current.get(id) || "";
}
...
export default function GalleryScreen() {
  ...
  async function ensureFolderExists(name: string) {
  ...
  // }
    ...
    if (folderValue) {
      await ensureFolderExists(folderValue);
    }
  ...
  async function createFolderAndSelect(name: string) {
    ...
    // гарантируем что папка будет в списке папок
...
/**
 * No-op placeholder to keep file structure stable if you diff a lot.
 * Remove anytime if you want (not required for functionality).
 */
function setArrowFixNoop() {}
